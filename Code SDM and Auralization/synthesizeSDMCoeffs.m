...
% H = synthesizeSDMCoeffs(P,DOA,varargin)fft
...
if p.Binaural
    ...
    disp(nfft);
    [rowsH, colH] = size(H);
    [rowsp, colp] = size(p.hrir_l);
    padCol = colp - colH;
    H = [H, zeros(size(H, 1), padCol)];
...
for n = 1:NN
    ...
    % Reference frame and NLS impulse response frame
    X_ref_temp = bsxfun(@times, win, x_ref(startx:endx)); 
    X_sdm_temp = bsxfun(@times, win, x_sdm(startx:endx)); 
    
    [row, col] = size(X_ref_temp);
    X_ref = zeros(row, col);

    for i = 1:col
        cur_col = X_ref_temp(:,i);
        %
        X_ref(:, i) = fft(cur_col); 
    end

    [row, col] = size(X_sdm_temp);
    X_sdm = zeros(row, col);

    for i = 1:col
        cur_col = X_sdm_temp(:,i);
        %
        X_sdm(:, i) = fft(cur_col); 
    end
    
    
    %X_ref = fft(X_ref_temp,nfft*2);
