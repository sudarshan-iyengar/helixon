...
%         p.LFEchannel = [];
        ...
        Az = linspace(0, 345, 24); % Degrees
        El = linspace(-90, 90, 12); % Degrees
...
if p.Binaural
    ...
    azimuths = 0:15:345; % Example: every 2 degrees from 0 to 358
    elevations = -90:15:90; % Example: every 2 degrees from -88 to 88
    ...
    % Resample HRIRs to match the system's sampling rate
    p.hrir_l = resample(hrir_l, p.fs, p.hrtfData.Data.SamplingRate);
    p.hrir_r = resample(hrir_r, p.fs, p.hrtfData.Data.SamplingRate)
end
if isempty(varargin)
